...
#Let's set up our linkedin Login
def linkedin_login():
    # Go to Linkedin home page
    _DRIVER_CHROME.get(parameters.linkedin_url)
    _DRIVER_CHROME.find_element_by_xpath('//a[text()="Sign in"]').click()
    sleep(5)

    # locate and populate login id text box
    username_input = _DRIVER_CHROME.find_element_by_name('session_key')
    username_input.send_keys(parameters.linkedin_username)
    sleep(5)

    # locate and populate password text box
    password_input = _DRIVER_CHROME.find_element_by_name('session_password')
    ...
    _DRIVER_CHROME.find_element_by_xpath('//button[text()="Sign in"]').click()


    print('LinkedIn login OK!')

#Let's set up our google search
def google_search():
